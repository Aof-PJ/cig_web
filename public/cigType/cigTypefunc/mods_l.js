...
function ButtonNextQ() {
    ...
    Ans = document.getElementsByName("Mods_B")
    for(ans of Ans){
        if(ans.checked){
            if(ans.value == "อื่นๆ"){
                ansOther = document.getElementById("otherMods").value
                if(ansOther != ""){
                    check1 = checkOther()
    ...
    Ans = document.getElementsByName("Mods_f")
    ...
    if(check1 && check2){
        ...
        Index = listName.indexOf("ModsLiquid");
        listAns[Index] = ans1
        Index = listName.indexOf("ModsLiquid_other");
